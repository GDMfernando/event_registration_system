  ...
  fetch(`manage_events.php?action=fetch_event&id=${eventId}`)
    ...
    .then((data) => {
      if (data.success) {
        ...
        document.getElementById("edit_price_regular").value =
          event.price_regular;
        document.getElementById("edit_price_balcony").value =
          event.price_balcony;
        ...
        if (event.status) {
          // We convert the DB status to lowercase to compare it safely
          const dbStatus = event.status.toLowerCase();

          // Loop through the dropdown options to find the match
          for (let i = 0; i < statusField.options.length; i++) {
            if (statusField.options[i].value.toLowerCase() === dbStatus) {
